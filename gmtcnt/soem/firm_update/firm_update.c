...
#define FWBUFSIZE (16384)

uint8 ob;
uint16 ow;
uint32 data;
char g_filename[256];
char g_filebuffer[FWBUFSIZE]; // 8MB buffer
int  g_filesize;
int  j;
uint16 argslave;
	...
	if (ec_init(ifname))
	{
	    ...
	    if ( ec_config_init(FALSE) > 0 )
		{
			...
			if (ec_statecheck(slave, EC_STATE_BOOT,  EC_TIMEOUTSTATE * 10) == EC_STATE_BOOT)
			{
				...
				if (input_bin(filename, &g_filesize))
				{
					printf("File read OK, %d bytes.\n", g_filesize);
					printf("FoE write....");
					j = ec_FOEwrite(slave, filename, 0, g_filesize , &g_filebuffer, EC_TIMEOUTSTATE);
					printf("result %d.\n",j);
					printf("Request init state for slave %d\n", slave);
					ec_slave[slave].state = EC_STATE_INIT;
					ec_writestate(slave);
