...
void eepromtool(char *ifname, int slave, int mode, char *fname)
{
   ...
   if (ec_init(ifname))
   {
      ...
      w = 0x0000;
       g_wkc = ec_BRD(0x0000, ECT_REG_TYPE, sizeof(w), &w, EC_TIMEOUTSAFE);      /* detect number of slaves */
       if (g_wkc > 0)
       {
         ec_slavecount = g_wkc;
         ...
         if((ec_slavecount >= slave) && (slave > 0))
         {
            if ((mode == MODE_INFO) || (mode == MODE_READBIN) || (mode == MODE_READINTEL))
            {
               ...
               printf(" Vendor ID        : %8.8X\n", (unsigned int) *(uint32 *)(wbuf + 0x08));
               printf(" Product Code     : %8.8X\n", (unsigned int) *(uint32 *)(wbuf + 0x0A));
               printf(" Revision Number  : %8.8X\n", (unsigned int) *(uint32 *)(wbuf + 0x0C));
               printf(" Serial Number    : %8.8X\n", (unsigned int) *(uint32 *)(wbuf + 0x0E));
               printf(" Mailbox Protocol : %4.4X\n", (unsigned int) *(wbuf + 0x1C));
			...
			{
			  if(eeprom_writealias(slave, g_alias))
...
int main(int argc, char *argv[])
{
   ...
   {
      g_slave = atoi(argv[2]);
      if ((strncmp(argv[3], "-i", sizeof("-i")) == 0))   g_mode = MODE_INFO;
	  if (argc > 4)
	  {
        if ((strncmp(argv[3], "-r", sizeof("-r")) == 0))   g_mode = MODE_READBIN;
        if ((strncmp(argv[3], "-ri", sizeof("-ri")) == 0)) g_mode = MODE_READINTEL;
        if ((strncmp(argv[3], "-w", sizeof("-w")) == 0))   g_mode = MODE_WRITEBIN;
        if ((strncmp(argv[3], "-wi", sizeof("-wi")) == 0)) g_mode = MODE_WRITEINTEL;
      ...
      /* start tool */
      eepromtool(argv[1], g_slave, g_mode, argv[4]);
   }
   else
   {
